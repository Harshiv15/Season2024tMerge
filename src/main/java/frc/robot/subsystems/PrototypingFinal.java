...
public class PrototypingFinal extends SubsystemBase {
    ...
    public void vPeriodic() {
        ...
        if(!motor1Sendable.closedLoopEnabled) {
            ...
            .setReference(((motor1Sendable.m_setpoint/(2 * (Math.PI)* 0.0508))*60), ControlType.kVelocity);
            else motor1.set(0.0);
        }
        ...
        if(!motor2Sendable.closedLoopEnabled) {
            ...
            if (motor2Sendable.openLoopEnabled) motor2.getPIDController()
            .setReference(((motor2Sendable.m_setpoint/(2 * (Math.PI)* 0.0508))*60), ControlType.kVelocity);
            else motor2.set(0.0);
        }
        if(!motor3Sendable.closedLoopEnabled) {
            ...
            if (motor3Sendable.openLoopEnabled) motor3.getPIDController()
            .setReference(((motor3Sendable.m_setpoint/(2 * (Math.PI)* 0.0508))*60), ControlType.kVelocity);
            else motor3.set(0.0);
        }
